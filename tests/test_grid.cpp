...
void test_birth_rule() {
    ...
    // Configuration : 3 cellules vivantes autour de (1,1)
    ...
    // Verifier que (1,1) est maintenant vivante
...
void test_survival_rule() {
    ...
    // Verifier que (1,1) reste vivante
    assert(grid.getCell(1, 1).isAlive());
    
    cout << "[OK] Test 4 : Survie avec 2-3 voisins" << endl;
}
...
// Test 6 : Bloc stable 2x2
void test_stable_block() {
    Grid grid(4, 4);
    StandardRule rule;
    
    // Configuration : bloc 2x2
    grid.getCell(1, 1).setState(new AliveState());
    ...
    // Verifier que le bloc n'a pas change
    assert(grid.getCell(1, 1).isAlive());
...
// Test 7 : Comptage des voisins
void test_neighbour_count() {
    Grid grid(3, 3);
    
    // Configuration : 8 voisins autour de (1,1)
    grid.getCell(0, 0).setState(new AliveState());
    grid.getCell(0, 1).setState(new AliveState());
    grid.getCell(0, 2).setState(new AliveState());
    grid.getCell(1, 0).setState(new AliveState());
...
void test_file_reading() {
    ...
    // Verifie que la grille a les bonnes dimensions
